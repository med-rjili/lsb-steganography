...
void cacher_texte(byte *pixels, int taille_pixels, char *texte) {
    ...
    for (i = 0; i < taille_texte; i++) {
        ...
        for (j = 0; j < 8; j++) {
            // Extraire le bit j du caractère
            bit = (texte[i] >> j) & 1;
            // Insérer le bit dans le pixel
            k = i * 8 + j;
            if (k+3 < taille_pixels) {
                pixels[k+3] = (pixels[k+3] & 0xFE) | bit;
            }
        }
    }
}
...
char *extraire_texte(byte *pixels, int taille_pixels) {
    ...
    for (i = 0; i < taille_texte; i++) {
        ...
        texte[i] = 0;
        // Parcourir chaque bit du caractère
        for (j = 0; j < 8; j++) {
            // Extraire le bit du pixel
            k = i * 8 + j;
            if (k+3 < taille_pixels) {
                bit = pixels[k+3] & 1;
                // Insérer le bit dans le caractère
                texte[i] |= bit << j;
            }
        }
    }
